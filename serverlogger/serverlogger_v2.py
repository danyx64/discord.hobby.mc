...
class ServerLogger(BaseServerLogger):
    """ServerLogger v1.1: invio piu affidabile e layout compatto/orizzontale."""
    ...
    def _init_message_cache(self):
        with self._connect() as db:
            ...
            db.execute(
                "CREATE INDEX IF NOT EXISTS idx_message_cache_guild ON message_cache(guild_id, message_id)"
            )
            db.commit()
    ...
    def _make_embed(self, guild, log_id, action, staffer_id, user_id, channel_id, when, details=None):
        ...
        embed.add_field(name="Staffer", value=self._display_member(guild, staffer_id), inline=True)
        embed.add_field(name="Utente", value=self._display_member(guild, user_id), inline=True)
        embed.add_field(name="Canale", value=self._display_channel(guild, channel_id), inline=True)
        ...
        if details:
            visible = [
                f"**{key}:** {self._clean_text(value, 350)}"
